...
export function Timeline({ events, tasks }: TimelineProps) {
  ...
  useEffect(() => {
    if (!openEventId && events[0]) setOpenEventId(events[0].id);
    if (openEventId && !events.some((event) => event.id === openEventId)) setOpenEventId("");
  }, [events, openEventId]);

  return (
    <section className="rounded-lg bg-white p-5 shadow-soft" id="timeline">
      <div>
        <p className="text-xl font-bold text-forest-700">以月份查看活動與任務分布</p>
                ...
                <div className="space-y-2">
                  ...
                  {isOpen &&
                    (eventTasks.length ? (
                      ...
                      <p className="rounded-md bg-rice px-4 py-3 text-lg font-black text-forest-800">
                        此活動尚未建立任務
                      </p>
                    ))}
                </div>
