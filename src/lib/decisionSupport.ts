...
import { getDaysLeft, isTaskClosed } from "./reminders";
...
export function getAssigneeIds(task: Task) {
...
export function calculatePriorityScore(task: Task, context: PriorityContext) {
  ...
  if (task.status === "review") score += 35;
  if (isTaskClosed(task)) score -= 999;
  ...
  assigneeIds.forEach((assigneeId) => {
    const unfinished = context.tasks.filter(
      (candidate) => !isTaskClosed(candidate) && getAssigneeIds(candidate).includes(assigneeId)
    );
  ...
  if (event) {
    ...
    if ((task.isCritical || task.isKeyTask) && !isTaskClosed(task)) score += 40;
  }
...
export function getPriorityReasons(task: Task, context: PriorityContext) {
  ...
  if (isTaskClosed(task)) return ["已處理"];
  ...
  if (task.status === "waiting") reasons.push("等待回覆");
  if (task.status === "review") reasons.push("待主任確認");
  if (task.isCritical || task.isKeyTask) reasons.push("關鍵任務");
  if (task.isBlocked) reasons.push("目前卡關");
  if (assigneeIds.length === 0) reasons.push("尚未指派");
  if (task.priority === "high") reasons.push("主任標優先");
  ...
  return tasks
    ...
    .map((task) => {
      ...
      return {
        ...
        taskId: task.id,
        task,
        eventName: event?.name ?? "一般任務",
        category,
        reason: `${task.title}：${category}`,
        suggestion:
          category === "未指派"
            ? "建議先指定負責人，避免任務沒有人接手。"
            : category === "延遲"
  ...
  const rows = teachers
    ...
    .map((teacher) => {
      const active = tasks.filter(
        (task) => !isTaskClosed(task) && getAssigneeIds(task).includes(teacher.id)
      );
...
export function getEventRisks(event: Event, tasks: Task[]) {
  const eventTasks = tasks.filter((task) => task.eventId === event.id);
  const blocking = eventTasks.find((task) => (task.isCritical || task.isKeyTask) && !isTaskClosed(task));
  const unassigned = eventTasks.find((task) => getAssigneeIds(task).length === 0 && !isTaskClosed(task));
  const delayed = eventTasks.find((task) => getDaysLeft(task.dueDate) < 0 && !isTaskClosed(task));

  return {
    blocking: blocking ? `${blocking.title.replace(`${event.name} - `, "")} 尚未完成` : "目前沒有明顯卡關",
    risk: unassigned
...
export function getWorkloadRows(teachers: Teacher[], tasks: Task[]) {
  return getTeacherLoadSuggestions(tasks, teachers).map((row) => ({
    ...
    active: tasks.filter((task) => !isTaskClosed(task) && getAssigneeIds(task).includes(row.teacher.id)),
    soon: tasks.filter((task) => {
      const days = getDaysLeft(task.dueDate);
      return !isTaskClosed(task) && getAssigneeIds(task).includes(row.teacher.id) && days >= 0 && days <= 3;
    }),
    overdue: tasks.filter(
      (task) => !isTaskClosed(task) && getAssigneeIds(task).includes(row.teacher.id) && getDaysLeft(task.dueDate) < 0
    )
  }));
}
...
export function balanceTaskAssignments(tasks: Task[], teachers: Teacher[]) {
  ...
  const activeCounts = new Map(
    teacherIds.map((id) => [
      id,
      tasks.filter((task) => !isTaskClosed(task) && getAssigneeIds(task).includes(id)).length
    ])
  );

  return tasks.map((task) => {
    if (isTaskClosed(task) || task.isCritical || task.isKeyTask) return task;
    ...
    return { ...task, assignees: [leastLoaded[0]], ownerIds: [leastLoaded[0]], assignedTo: leastLoaded[0], updatedAt: getTodayString() };
  });
}
